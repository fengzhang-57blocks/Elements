...
class CalendarPagingCell: PagingCell {
  ...
  override func setItem(_ item: PagingItem, selected: Bool, options: PagingOptions) {
    self.options = options
    let calendarItem = item as! CalendarItem
  ...
  override func apply(_ layoutAttributes: UICollectionViewLayoutAttributes) {
    ...
    if let attributes = layoutAttributes as? PagingCellLayoutAttributes {
      dateLabel.textColor = UIColor.interpolate(
        ...
        percentage: attributes.progress
      )
